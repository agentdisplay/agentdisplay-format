...
The second group is a hard requirement because of what a display is: a public capability URL whose freeform params are *persisted* and *publicly rendered*. A redirect that appends `?token=…` — an OAuth callback, a checkout success URL — would otherwise write a live credential into storage and publish it on the page. This is not hypothetical; it is why the rule exists. A secret that never reaches the database cannot leak from it.
